...
from diffusion_model.trainer_mps import GaussianDiffusion, Trainer
...
if torch.backends.mps.is_available():
    ...
    print("MPS device found. Using MPS for training.")
elif torch.cuda.is_available():
    device = torch.device("cuda")
    print("CUDA device found. Using CUDA for training.")
else:
...
parser.add_argument('-t', '--targetfolder', type=str, default="dataset/vs_segimage/")
parser.add_argument('--input_size', type=int, default=512)
parser.add_argument('--depth_size', type=int, default=128)  # Changed from 120 to 128
parser.add_argument('--num_channels', type=int, default=64) # Must be divisible by 32 for GroupNorm 
...
diffusion = GaussianDiffusion(
    model, #unet model (renamed denoise_fn in the trainer_mps.py)
...
trainer = Trainer(
    diffusion, #diffusion model object 
    dataset, #dataset object made with  NiftiPairImageGenerator
    image_size = input_size,
    depth_size = depth_size,
