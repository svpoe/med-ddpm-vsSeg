...
# Modified for MPS (Apple Silicon) compatibility
...
# MPS doesn't support APEX, so we disable it
APEX_AVAILABLE = False
print("APEX: OFF (Not compatible with MPS)")
...
def loss_backwards(fp16, loss, optimizer, **kwargs):
    # MPS doesn't support mixed precision, so we just use regular backward
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, c = None):
        # HERE DURING INFERENCE, denoise_fn predicts noise to reconstruct clean image
    ...
    def p_losses(self, x_start, t, condition_tensors = None, noise = None):
        ...
        # HERE: IN FORWARD PASS, denoise_fn (U-Net) predicts the noise
        if self.with_condition:
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
            # Concatenate noisy image + condition (mask)
            # U-Net called here!
...
class Trainer(object):
    def __init__(
        ...
        self.ds = dataset
        # For MPS, we reduce num_workers to avoid potential issues
        ...
        # Disable fp16 for MPS as it's not supported
        if self.device.type == "mps":
            self.fp16 = False
            print("Mixed precision disabled for MPS compatibility")
        else:
            self.fp16 = fp16
